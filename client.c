...
#include <unistd.h>
    ...
    if (argc != 3) {
        printf("\nUsage: tcp-client <IPaddress> <Port Number>");
        exit(0);
    }
    ...
    printf("Please enter the URL:PortNo:FileName:");
    bzero(buffer,256);
    str_cli(sockfd, argv);
}
    ...
    /* send it to proxy server */
    if (write(sockfd, (const void*) sendline, strlen(sendline)) < 0) {
        printf("\n%s: Error in write", argv[0]);
        exit(0);
    }
    
    /* get from the proxy server and display it on the terminal */
